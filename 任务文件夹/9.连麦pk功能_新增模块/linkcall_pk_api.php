    ...
    public static function on_linkcallpk_user_send_gift_event($pkid,$user_id,$giftall,$singer_id)
    {
        ...
        if (linkcall_pk_model::$LINKCALL_PK_SET_CONTROL)
        {
            ...
            do
            {
                ...
                //$identity 主播标志位是2
                $identity = (int)$get_user_info["identity"];
                if (2 == $identity)
                {
                    //如果是主播
                    $user_cache['user_wealth'] = (int)$get_user_level['experience_level'];
                    $user_cache['user_level']  = (int)$get_user_level['active_level'];

                }
                else
                {
                    //如果是普通用户
                    $user_cache['user_wealth'] = (int)$get_user_level['consume_level'];
                    $user_cache['user_level']  = (int)$get_user_level['active_level'];
                }

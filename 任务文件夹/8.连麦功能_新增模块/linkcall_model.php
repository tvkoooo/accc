...
{ 
    ...
    //Linkcall 常量：  
    public static $LINKCALL_SET_MSQ_APPLY    = 0;//设置控制门，启用连麦不成功,写入mysql
    ...
    public static $LINKCALL_APPLY_MAX_PLAYER = 10;//最大连麦申请个数    10个
    public static $LINKCALL_APPLY_MAX_TIME    = 30;//用户最大申请保留时间 30s
    ...
    public function set_singer_linkcall_state(&$error,$sid,$linkcall_state)
    {
        ...
        do
        {
            ...
            if(null == $redis)
            {
                // 100000701(701)网络数据库断开连接，返回给客户端统一一个错码
    ...
    //3.6a   redis 读出     房间内规定时间内用户连麦申请用户列表
    public function get_user_apply_index(&$error,$sid,&$apply_list)
    {
        $error['code'] = -1;
        $error['desc'] = '未知错误';
        $time_now = time();
        $time_30s_ago = $time_now - linkcall_model::$LINKCALL_APPLY_MAX_TIME;
        do
        {
            $redis = $this->getRedisMaster();
            if(null == $redis)
            ...
            $key = linkcall_model::linkcall_user_data_apply_zset_key($sid);
            $get_apply_list = $redis->zRangeByScore($key,$time_30s_ago,$time_now);
    ...
    //3.6b   redis 查询     房间内用户是否在30s时间申请列表
    public function find_user_apply_index(&$error,$sid,$user_id)
    {
        ...
        $is_apply = false;
        $time_now = time();
        //备注：需要一个3s的服务器误差延时计算，（防止客户端在30s结束后发过来的被动拒绝请求超出申请列表里面用户存在时间，导致申请列表无用户数据）
        $time_30s_ago = $time_now - linkcall_model::$LINKCALL_APPLY_MAX_TIME - 3;
        do
        {
            $redis = $this->getRedisMaster();
            if(null == $redis)
            ...
            $key = linkcall_model::linkcall_user_data_apply_zset_key($sid);
            $time_apply = $redis->zScore($key,$user_id);
            //取出申请时间比30+3s前要小，说明这个用户不在30+3 s（%10延时误差时间）内了
            if ($time_apply < $time_30s_ago)
            {
                $is_apply =false;
    ...
    public function get_user_apply_index_count(&$error,$sid)
    {
        ...
        do
        {
            ...
            $key = linkcall_model::linkcall_user_data_apply_zset_key($sid);
            $v = $redis->zCount($key,$time_30s_ago,$time_now);
            if(true == empty($v))
            {
                //如果取出无数据，给个default 值 0，代表列表无数据
                $v = 0;
 ...
 ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////   
    ...
    public function user_apply_apply_linkcall(&$error,&$return,$sid,$singer_id,$singer_nick,$user_id,&$data_cache,&$linkcall_apply,&$linkcall_state)
    {
        ...
        do
        {
            ...
            if (1 == $is_link)
            {
                // 403300022(022)用户在连接列表，用户本次操作无效，退出返回rs
                $error['code'] = 403300022;
                $error['desc'] = '';
                LogApi::logProcess("user_apply_apply_linkcall error:".json_encode($error));
            ...
            if (true == $is_apply)
            {
                // 403300016(016)用户在申请列表，用户在连接列表，用户本次操作无效，退出返回rs
            ...
            // 4 记录用户
            {
                ...
                //重置该用户连麦允许时间为0
                $this->set_user_link_time(&$error,$sid,$user_id,0);
                if (0 != $error['code'])
                {
                    //出现了一些逻辑错误
                    break;
    ...
    { 
        ...
        do
        {
            ...
            if (false == $is_apply)
            {
                // 403300019(019)用户不在申请列表，用户本次操作无效
                $error['code'] = 403300019;
                $error['desc'] = '';
                LogApi::logProcess("user_apply_desapply_linkcall error:".json_encode($error));
                break;
            }  
    ...
    public function user_apply_out_linkcall(&$error,&$return,$sid,$singer_id,$singer_nick,$user_id,&$linkcall_apply,&$linkcall_state)
    {
        ...
        do
        {
            ...
            if (0 == $is_link)
            {
                ...
                // 403300020(020)该用户不在链接列表，用户本次操作无效
                $error['code'] = 403300020;
                $error['desc'] = '';
                LogApi::logProcess("user_apply_out_linkcall error:".json_encode($error));
                break;
            }      
    ...
    //6.4   用户30s被动取消申请连麦
    public function user_apply_desapply_30s_linkcall(&$error,&$return,$sid,$singer_id,$singer_nick,$user_id,&$linkcall_apply,&$linkcall_state)
    {
        $error['code'] = -1;
        $error['desc'] = '未知错误';
        do
        {
            ...
            // 4.1 发送用户取消连麦，单播给主播nt（发给主播的连麦状态还是用户取消）
            //  登记用户取消申请状态
            $linkcall_apply_singer = linkcall_model::$LINKCALL_APPLY_DESAPPLY;
            $this->set_user_apply_state(&$error,$sid,$user_id,$linkcall_apply_singer);
            if (0 != $error['code'])
            {
                //出现了一些逻辑错误
                break;
            }
            // 发送用户取消连麦，单播给主播nt（发给主播的连麦状态还是用户取消）
            $this->linkcall_apply_singer_nt(&$error,&$return,$sid,$singer_id,$singer_nick,$linkcall_state,$user_id);
            if (0 != $error['code'])
            {
                //出现了一些逻辑错误
            ...
            // 4.2 设置用户被主播拒绝
            $linkcall_apply_singer = linkcall_model::$LINKCALL_APPLY_NO;
            $this->set_user_apply_state(&$error,$sid,$user_id,$linkcall_apply_singer);
            if (0 != $error['code'])
            {
                //出现了一些逻辑错误
        ...
        if (0 !=$error['code'])
        {
            LogApi::logProcess("user_apply_desapply_30s_linkcall error:".json_encode($error));
        }
    }
    ...
    public function singer_apply_yes_linkcall(&$error,&$return,$sid,$singer_id,$singer_nick,$user_id,&$linkcall_state,&$linkcall_apply)
    {
        ...
        do
        {
            ...
            if (false == $is_apply)
            {
                ...
                if (0 == $is_link)
                {
                    ...
                    $error['code'] = 403300019;
                    $error['desc'] = '信号不好，他申请结束了';
                    break;
                }
                else
                {
                    //403300022(022)用户在连麦列表，
                    $error['code'] = 403300022;
                    $error['desc'] = '他正连着麦';
                    break;
                }
            ...
            else
            {
                ...
                //对比允许时间-申请时间是否超过30s,超过30s，用户取消连麦连麦申请 ,并回复用户已经不在申请列表
                if ($time_allow-$time_apply > linkcall_model::$LINKCALL_APPLY_MAX_TIME)
                {
                    $linkcall_apply_this = linkcall_model::$LINKCALL_APPLY_DESAPPLY;
                    $this->user_apply_desapply_30s_linkcall(&$error,&$return,$sid,$singer_id,$singer_nick,$user_id,&$linkcall_apply_this,&$linkcall_state);
                    //403300019(019)用户不在申请列表
                    $error['code'] = 403300019;
                    $error['desc'] = '信号不好，他申请结束了';
                    break;
        ...
        if (0 !=$error['code'])
        {
            LogApi::logProcess("singer_apply_yes_linkcall error:".json_encode($error));
        }
    }
    ...
    public function singer_apply_no_linkcall(&$error,&$return,$sid,$singer_id,$singer_nick,$user_id,&$linkcall_state,&$linkcall_apply)
    {
        ...
        if (0 !=$error['code'])
        {
            LogApi::logProcess("singer_apply_no_linkcall error:".json_encode($error));
        }
    ...
    public function singer_apply_del_linkcall(&$error,&$return,$sid,$singer_id,$singer_nick,$user_id,&$linkcall_state,&$linkcall_apply)
    {
        ...
        do
        {
            ...
            if (0 == $is_link)
            {
                // 403300020(020)用户不在连接列表
                $error['code'] = 403300020;
                $error['desc'] = '信号不好，他断开了连麦';
                break;
            }
        ...
        if (0 !=$error['code'])
        {
            LogApi::logProcess("singer_apply_del_linkcall error:".json_encode($error));
        }
    }
    ...
    public function linkcall_userdata_by_uid(&$error,$sid,$user_id,&$data)
    {
        ...
        if (0 !=$error['code'])
        {
            LogApi::logProcess("linkcall_userdata_by_uid error:".json_encode($error));
        }
    }    
    ...
    public function linkcall_link_all_user_datas(&$error,$sid,&$datas)
    {
        ...
        if (0 !=$error['code'])
        {
            LogApi::logProcess("linkcall_link_all_user_datas error:".json_encode($error));
        }
    }
    ...
    public function linkcall_apply_all_user_datas(&$error,$sid,&$datas)
    {
        ...
        if (0 !=$error['code'])
        {
            LogApi::logProcess("linkcall_apply_all_user_datas error:".json_encode($error));
        }
    }
    ...
    public function linkcall_room_state_nt(&$error,&$return,$sid,$singer_id,$singer_nick,$linkcall_state)
    {
        ...
        if (0 !=$error['code'])
        {
            LogApi::logProcess("linkcall_room_state_nt error:".json_encode($error));
        }
    }
    ...
    public function linkcall_user_state_nt(&$error,&$return,$sid,$singer_id,$singer_nick,$linkcall_state,$user_id)
    {
        ...
        if (0 !=$error['code'])
        {
            LogApi::logProcess("linkcall_user_state_nt error:".json_encode($error));
        }
    }
    ...
    public function linkcall_apply_singer_nt(&$error,&$return,$sid,$singer_id,$singer_nick,$linkcall_state,$user_id)
    {
        ...
        if (0 !=$error['code'])
        {
            LogApi::logProcess("linkcall_apply_singer_nt error:".json_encode($error));
        }
    }
    ...
    public function linkcall_mysql_log_desapply(&$error,$sid,$singer_id,$user_id)
    {
        if (linkcall_model::$LINKCALL_SET_MSQ_APPLY)
        {
            ...
            if (0 !=$error['code'])
            {
                LogApi::logProcess("linkcall_mysql_log_desapply error:".json_encode($error));
            }
        }
    }
    ...
    public function linkcall_mysql_log_link_over(&$error,$sid,$singer_id,$user_id)
    {
        if (linkcall_model::$LINKCALL_SET_MSQ_ALLOW)
        {
            ...
            if (0 !=$error['code'])
            {
                LogApi::logProcess("linkcall_mysql_log_link_over error:".json_encode($error));
            }
        }

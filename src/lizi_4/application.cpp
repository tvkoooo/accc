...
{	
	//lizhi4_wangluo_fw_accept_fuwu_open(&p->wk1);
	wangluo_fw_accept_start(&p->fw1);
...
{	
	//lizhi4_wangluo_fw_accept_fuwu_open(&p->wk1);
	wangluo_fw_accept_poll_wait(&p->fw1);
...
{
	wangluo_fw_accept_interrupt(&p->fw1);
...
void application_shutdown(struct application* p)
{
	...
	wangluo_fw_accept_shutdown(&p->fw1);
	//wangluo_kh_shutdown(&p->kh1);

}
void application_join(struct application* p)
{
	wangluo_fw_accept_join(&p->fw1);
	//wangluo_kh_join(&p->kh1);

	//lizhi4_wangluo_fw_accept_fuwu_close(&p->wk1);
}

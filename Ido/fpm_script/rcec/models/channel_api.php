...
{    
    ...
    {        
        ...
        do
        {
            ...
            //触发夺旗主播/用户出场流程
            {
                $leave = new flag_faction_model();
                //判断是否是主播
                if ($singer_id == $uid)
                {
                    //是主播，触发主播离场
                    $leave->event_singer_room_leave(&$return, $singer_id, $sid);
                }
                else
                {
                    //是用户，触发用户离场
                    $leave->event_user_room_leave(&$return, $uid, $sid);
            ...
            //触发连麦主播PK 离场流程
            {
                $linkcallpk = new linkcall_pk_api();
                //判断是否是主播
                if ($singer_id == $uid)
                {
                    //是主播，触发主播离场
                    $linkcallpk->on_linkcallpk_singer_leave_event($params);
                }
	...
	public static function on_p_channel_heartbeat_event($params)
	{
	    ...
	    do
	    {
            ...
            // 人气值更新
            {
                $popularity = isset($params['popularity']) ? intval($params['popularity']) : 0;

                if ($popularity > 0) {
                    $model_channel = new ChannelLiveModel();
                    $model_channel->update_popularity($sid, $popularity);
                }
            }

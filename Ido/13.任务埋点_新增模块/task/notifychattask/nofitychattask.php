...
function call($params, $postdata)
{
	...
	logs::addLog("INFO::taskinfo::nofitychattask::不知道是否存在的任务  params:$params ", $logfile);
	...
	if (!property_exists($jparams,"uid")) {
	    ...
	    logs::addLog("WARN::taskinfo::nofitychattask::不知道是否存在的任务  params lost uid ", $logfile);
	...
	//logs::addLog($logs_array, $logfile);
    logs::addLog("INFO::taskinfo::nofitychattask::不知道是否存在的任务 uid:$uid retparams:".json_encode($retparams), $logfile);

...
function call($params, $postdata)
{
	...
	logs::addLog("INFO::taskinfo::getlooptasklist::获取跑环任务列表  params:$params ", $logfile);
	...
	if (!property_exists($jparams,"uid")) {
	    ...
	    logs::addLog("WARN::taskinfo::getlooptasklist::获取跑环任务列表  params lost uid ", $logfile);
	...
	if( !(0 == $uid || is_null($l_cur_num ) || is_null($h_cur_num ))){
		...
		if($l_cur_num >= $l_total_num && $h_cur_num > $h_total_num){
		    ...
		    logs::addLog("INFO::taskinfo::getlooptasklist::获取跑环任务列表   uid:$uid 已经完成所有跑环任务。return:".json_encode($retparams), $logfile);
		    return json_encode($retparams);
		}
		...
		if($rows_1){
		    ...
		    //logs::addLog("DEBUG::taskinfo::getlooptasklist::获取跑环任务列表   uid:$uid 拥有的刷新卡数量 num:$t_num", $logfile);
		    $retparams->toolNum = $t_num;
		}
		else {
		    logs::addLog("WARN::taskinfo::getlooptasklist::获取跑环任务列表   uid:$uid SQL_FAIL=>sql_1:$sql_1", $logfile);
		}
		
		//获取该轮跑环任务的奖励详情
		$sql_2 = "select t.sun, t.leaf, r.sun_revise, r.leaf_revise, p.parm1 as leaf_top, 
		...
		if($rows_2 && $row = $db->fetch_assoc($rows_2)){
		    $sun = (int)$row['sun'];
		    $leaf = (int)$row['leaf'];
		    ...
		    $error_sql = $db->error();
		    logs::addLog("WARN::taskinfo::getlooptasklist::获取跑环任务列表   uid:$uid error_sql:$error_sql  SQL_FAIL=>sql_2:$sql_2", $logfile);
		}

	}else{
		logs::addLog("ERROR::taskinfo::getlooptasklist:: 获取跑环任务列表   uid:$uid  uid==0  or user has no init looptask", $logfile);
	}
	...
	//获取该轮跑环任务的详情
	$query_3 = "select t.id, tc.task_name, tc.task_sketch,tc.task_type, tc.target_type, tc.award_goods_num,
	...
	if($rows_3){
		...
		//logs::addLog($logs_array, $logfile);	
		logs::addLog("INFO::taskinfo::getlooptasklist::获取跑环任务列表  uid:$uid  retparams:".json_encode($retparams), $logfile);

		return json_encode($retparams) ;
	}else{
	    logs::addLog("WARN::taskinfo::getlooptasklist::获取跑环任务列表  uid:$uid gangid:$gangid SQL_FAIL=>query_3:$query_3 ", $logfile);
		$db->disconnect() ;
		return false ;
	}
}

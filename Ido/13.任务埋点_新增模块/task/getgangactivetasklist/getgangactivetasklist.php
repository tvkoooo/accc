...
function call($params, $postdata)
{
	...
	logs::addLog("INFO::taskinfo::getgangactivetasklist::获得帮会活跃任务列表  params:$params ", $logfile);
	...
	if (!property_exists($jparams,"uid")) {
		...
		logs::addLog("WARN::taskinfo::getgangactivetasklist::获得帮会活跃任务列表  params lost uid ", $logfile);
	...
	{	    
	    ...
	    {
	        logs::addLog("WARN::taskinfo::getgangactivetasklist::获得帮会活跃任务列表  uid:$uid  SQL_FAIL=>sql_1:$sql_1 ", $logfile);
	    }
	}
	...
	if(empty($gangid)){
	    ...
	    //logs::addLog($logs_array, $logfile);
	    logs::addLog("INFO::taskinfo::getgangactivetasklist::获得帮会活跃任务列表 uid:$uid empty(gangid) return data=>:".json_encode($retparams), $logfile);  
	    return json_encode($retparams) ;
	}
	...
	$query_2 = "select t.id, tc.task_name, tc.task_sketch, tc.target_type, tc.key, 
			...
			where t.uid = $gangid and t.t_type = 7 and t.create_time = '$date'" ;
	...
	if($rows_2){
		...
		while($row = $db->fetch_assoc($rows_2)){
			...
			if($status == 2){
			    $status = 4;		//获得帮会活跃任务列表如果已领取奖励，则改成已完成
			}else{
		...
		//$logs_array = array("return data., ret"=>json_encode($retparams));
		//logs::addLog($logs_array, $logfile);				
		logs::addLog("INFO::taskinfo::getgangactivetasklist::获得帮会活跃任务列表  uid:$uid gangid:$gangid retparams:".json_encode($retparams), $logfile);
		return json_encode($retparams);
	}else{
	    logs::addLog("WARN::taskinfo::getgangactivetasklist::获得帮会活跃任务列表 uid:$uid gangid:$gangid SQL_FAIL=>query_2:$query_2", $logfile);
		$db->disconnect() ;
		return false ;
	}
}

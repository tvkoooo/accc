...
function call($params, $postdata)
{
	...
	logs::addLog("INFO::taskinfo::getgangmangifttasklist::获得帮会个人礼物任务列表  params:$params ", $logfile);
	...
	if (!property_exists($jparams,"uid")) {
	    ...
	    logs::addLog("WARN::taskinfo::getgangmangifttasklist::获得帮会个人礼物任务列表  params lost uid ", $logfile);
	...
	if($row = $db->fetch_assoc($rows_1)){
	    ...
	    logs::addLog("WARN::taskinfo::getgangmangifttasklist::获得帮会个人礼物任务列表 uid:$uid SQL_FAIL=>sql_1:$sql_1 ", $logfile);
	}
	...
	if(empty($gangid)){
	    ...
	    //logs::addLog($logs_array, $logfile);
	    logs::addLog("INFO::taskinfo::getgangmangifttasklist::获得帮会个人礼物任务列表  uid:$uid  empty(gangid) return data=>:".json_encode($retparams), $logfile);
	...
	$query_3 = "select t.id, tc.task_name, tc.task_sketch, tc.target_type, tc.award_goods_num, 
			...
			where t.uid = $jparams->uid and t.t_type = 14 and t.create_time = '$date'" ;
	...
	if($rows_3){
		...
		while($row = $db->fetch_assoc($rows_3)){
			...
			if($status == 2){			//帮会礼物任务如果已领取奖励，则改成已完成
			    $status = 4;
			}else{
		...
		//$logs_array = array("return data., ret"=>json_encode($retparams));
		//logs::addLog($logs_array, $logfile);
		logs::addLog("INFO::taskinfo::getgangmangifttasklist::获得帮会个人礼物任务列表 uid:$uid gangid:$gangid retparams:".json_encode($retparams), $logfile);
		
		return json_encode($retparams) ;
	}else{
	    logs::addLog("WARN::taskinfo::getgangmangifttasklist::获得帮会个人礼物任务列表 uid:$uid gangid:$gangid SQL_FAIL=>query_3:$query_3 ", $logfile);
		$db->disconnect() ;		
		return false ;
	}
}

...
function call($params, $postdata)
{
	...
	logs::addLog("INFO::taskinfo::getganggifttasklist::获得帮会礼物任务列表 params:$params ", $logfile);
	...
	if (!property_exists($jparams,"uid")) {
	    ...
	    logs::addLog("WARN::taskinfo::getganggifttasklist::获得帮会礼物任务列表 params lost uid ", $logfile);
	...
	if(empty($gangid))
	{
	    ...
	    {
	        logs::addLog("WARN::taskinfo::getganggifttasklist::获得帮会礼物任务列表 uid:$uid  SQL_FAIL=>sql_1:$sql_1 ", $logfile);
	    }
	}
	...
	if(empty($gangid)){
	    ...
	    //logs::addLog($logs_array, $logfile);
	    logs::addLog("INFO::taskinfo::getganggifttasklist::获得帮会礼物任务列表  uid:$uid  empty(gangid) return data=>:".json_encode($retparams), $logfile);
	    return json_encode($retparams) ;
	}
	...
	$query_2 = "select t.id, tc.task_name, tc.task_sketch, tc.target_type, tc.key, 
			...
			where t.uid = $gangid and t.t_type in(6, 15) and t.create_time = '$date' order by t.t_type desc" ;
	...
	if($rows_2){
		...
		//$logs_array = array("return data., ret"=>json_encode($retparams));
		//logs::addLog($logs_array, $logfile);
		logs::addLog("INFO::taskinfo::getganggifttasklist::获得帮会礼物任务列表 uid:$uid gangid:$gangid retparams:".json_encode($retparams), $logfile);
		
		return json_encode($retparams) ;
	}else{
	    
	    logs::addLog("WARN::taskinfo::getganggifttasklist::获得帮会礼物任务列表 uid:$uid gangid:$gangid SQL_FAIL=>query_2:$query_2 ", $logfile);
		$db->disconnect() ;
		return false ;
	}
}

...
function call($params, $postdata)
{
	...
	logs::addLog("INFO::taskinfo::getgangmanactivetasklist::获得帮会个人活跃任务列表  params:$params ", $logfile);
	...
	if (!property_exists($jparams,"uid")) {
	    ...
	    logs::addLog("WARN::taskinfo::getgangmanactivetasklist::获得帮会个人活跃任务列表 params lost uid ", $logfile);
	...
	{
	    logs::addLog("WARN::taskinfo::getgangmanactivetasklist::获得帮会个人活跃任务列表 uid:$uid SQL_FAIL=>sql_1:$sql_1 ", $logfile);
	}
	if(empty($gangid)){
	    ...
	    //logs::addLog($logs_array, $logfile);
	    logs::addLog("INFO::taskinfo::getgangmanactivetasklist::获得帮会个人活跃任务列表  uid:$uid  empty(gangid) return data=>:".json_encode($retparams), $logfile);
	    return json_encode($retparams) ;
	}
	...
	$query_3 = "select t.id, tc.task_name, tc.task_sketch, tc.target_type, tc.award_goods_num, 
	        ...
	        g.goods_icon, r.folder_path, u.status, ui.union_up_level as union_level, ul.honor_level from card.task_info t 
			...
			where t.uid = $jparams->uid and t.t_type in (18,12) and t.create_time = '$date' ORDER BY t.t_type DESC" ;
	...
	if($rows_3){
		...
		//$logs_array = array("return data., ret"=>json_encode($retparams));
		//logs::addLog($logs_array, $logfile);
		logs::addLog("INFO::taskinfo::getgangmanactivetasklist::获得帮会个人活跃任务列表 uid:$uid gangid:$gangid retparams:".json_encode($retparams), $logfile);
		return json_encode($retparams) ;
	}else{
	    logs::addLog("WARN::taskinfo::getgangmanactivetasklist::获得帮会个人活跃任务列表 uid:$uid gangid:$gangid SQL_FAIL=>query_3:$query_3 ", $logfile);
		$db->disconnect() ;
		return false ;
	}
}

...
class UserApi
{
     ...
     public static function enterChannel($params)
    {
        ...
        //新增坐骑特效（规则：有守护先守护，无守护查看是否有小天使，都没有查看最优坐骑（坐骑道具的最大id显示））
        $effect_id = 0;//初始化坐骑特效
        if (!(1 == $guardType_tianshi ||2 == $guardType_tianshi ||3 == $guardType_tianshi ||33 == $guardType_tianshi ))
        {            
            $zuoqi_modle = new Zuoqi_model;
            $zuoqiid = 0;
            $goods_info = array();
            $zuoqi_modle->redis_find_user_zuoqi_goodid($uid,&$zuoqiid,&$goods_info);            
            if(0 !=$zuoqiid)
            {
                //临时策略，客户端没有资源前用大小天使代替
                if (149 == $zuoqiid)
                {
                    $guardType_tianshi = 3;
                    $isguard = 1;
                ...
                {
                    $guardType_tianshi = 33;
                }
                $effect_id = $goods_info["effect_id"];
            }    
            LogApi::logProcess("UserApi::enterChannel: zuoqiid:$zuoqiid guardType_tianshi:$guardType_tianshi uid:$uid isguard:$isguard effect_id:$effect_id");
        } 
...
//         $activeManInfo = $userAttrModel->getActiveLevel($userAttr['active_point'], $uid);
       ...
       // }
       // file_put_contents("/data/phplog/1.log", "enterChannel end ===$nick==\n", FILE_APPEND);
    ...
    public static function getUserDetailInfoList($params){
        ...
        //file_put_contents("/data/phplog/1.log", "getUserDetailInfoList json:". $jsonStr ."\n", FILE_APPEND);
        ...
        //file_put_contents("/data/phplog/1.log", "getUserDetailInfoList useridlist:". $useridlist ."\n", FILE_APPEND);
        foreach ($useridlist as $userid){
        	//file_put_contents("/data/phplog/1.log", "getUserDetailInfoList userid:". $userid ."\n", FILE_APPEND);
        	$userAttr = array();
        	...
        	//file_put_contents("/data/phplog/1.log", "getUserDetailInfoList userid:". $userid . ",nick:". $userAttr['nick'] ."\n", FILE_APPEND);
    	}
        ...
        //file_put_contents("/data/phplog/1.log", "getUserDetailInfoList count:". $uidcount ."\n", FILE_APPEND);
        // 返回结果
        return $return;
    }
